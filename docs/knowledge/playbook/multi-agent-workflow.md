...
- Does the [ticket loop](/playbook/ticket-loop.md) heartbeat — one stateless prompt advancing every ticket by one step per tick — stay a sibling of per-issue orchestration, or eventually subsume it (the loop spawning a pipeline orchestrator as one of its steps)?
