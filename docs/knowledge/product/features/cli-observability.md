...
- **Boundary** — in scope. Both commands are pure CLI surface reading the two things the engine already exposes: the [Session](/platform/engine.md#the-engine) record (`GET /api/sessions/:id`, incl. `status`, `connectors`, bound `mounts`) and the append-only [Event Stream](/platform/engine.md#observation) (`GET /api/sessions/:id/events`, SSE replay + live). No new or changed primitive; no UI in-core, no orchestration, no vendor knowledge[^boundary].
...
- As an agent (dogfooding itself), I want the observability surface to be scriptable from the CLI, so a session can watch its own children end-to-end via the [Platform Connector](/product/features/platform-connector.md) and the [Event Stream](/platform/engine.md#observation).
...
- Both commands are covered by CLI tests (`packages/cli/src/lib/*.test.ts`, vitest) and documented in `docs/knowledge/platform/cli.md`.
...
- **`al status` does not exist.** It is absent from the `COMMANDS` set in `lib/args.ts`, from the `USAGE` block, and from `docs/knowledge/platform/cli.md`. This is the additive part: a new command that reads the session record plus the replayed event log.
...
- **Docs path.** The issue says "documented in `docs/reference/platform/cli.md`" but the CLI docs actually live at `docs/knowledge/platform/cli.md` (`docs/reference/` does not exist). Use the real path `docs/knowledge/platform/cli.md`; confirm this with the design gate[^cli].
