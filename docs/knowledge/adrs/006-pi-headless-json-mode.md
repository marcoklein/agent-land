...
> **Superseded by [the engine](/platform/engine.md)** — the engine now runs pi as `pi --mode rpc` (one engine opinion), not `--mode json --print`. Retained for decision history only.
